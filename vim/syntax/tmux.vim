...
\ main-pane-width menu-border-lines menu-border-style menu-selected-style
...
\ prefix-timeout prefix2 prompt-history-limit remain-on-exit
\ remain-on-exit-format renumber-windows repeat-time scroll-on-clear
\ session-closed session-created session-renamed session-window-changed
...
\ window-layout-changed window-linked window-pane-changed window-renamed
\ window-resized window-size window-status-activity-style
